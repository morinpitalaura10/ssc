...
const CrudUKM = ({ navigation }) => {
    ...
    const handleDelete = (row) => {
        Alert.alert(
            "Hapus UKM",
            `Yakin hapus "${row.nama_ukm || "UKM ini"}"?`,
            [
    ...
    // ✅ logo pakai style avatar yang sama biar konsisten card
    const Logo = ({ img }) => {
    ...
    const UkmCard = ({ item }) => (
        <View style={GlobalStyles.mhsCard}>
            <Logo img={item.logo_ukm} />

            <View style={GlobalStyles.mhsInfo}>
                <Text style={GlobalStyles.mhsName}>{item.nama_ukm || "-"}</Text>
                ...
                <Text style={GlobalStyles.mhsMeta}>
                    Anggota: {item.jumlah_anggota ?? "-"} • Latihan: {item.tempat_latihan || "-"}
                </Text>
                ...
                <View style={GlobalStyles.mhsActionsRow}>
                    <TouchableOpacity
                        ...
                        activeOpacity={0.8}
                        onPress={() => navigation.navigate("ReadUKM", { id_ukm: item.id_ukm })}
                    >
                    ...
                    <TouchableOpacity
                        style={GlobalStyles.mhsActionBtn}
                        activeOpacity={0.8}
                        onPress={() => navigation.navigate("UpdateUKM", { id_ukm: item.id_ukm })}
                    >
    ...
    return (
        <View style={GlobalStyles.container}>
            <HeaderPrimary title="KELOLA UKM" />

            <View style={GlobalStyles.stickyTopBar}>
                <TouchableOpacity
                    style={GlobalStyles.addBtn}
                    activeOpacity={0.85}
                    onPress={() => navigation.navigate("CreateUKM")}
                >
                    <Ionicons name="add-circle" size={20} color="#fff" />
                    <Text style={GlobalStyles.addBtnText}>Tambah UKM</Text>
                </TouchableOpacity>
            ...
            {loading ? (
                ...
                <FlatList
                    data={data}
                    keyExtractor={(item) => String(item.id_ukm)}
                    renderItem={({ item }) => <UkmCard item={item} />}
